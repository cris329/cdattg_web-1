/**
 * Script específico para la página moderna de fichas
 * Basado en la arquitectura de los módulos modernos del sistema
 */
document.addEventListener('DOMContentLoaded', () => {
    console.log('🚀 Fichas - Página moderna inicializada');

    // Sistema de notificaciones toast
    window.showToast = function(type, message) {
        // Buscar solo el primer toast dentro de vista-fichas para evitar duplicados
        const toast = document.querySelector('.vista-fichas .toast.toast-minimal');
        if (!toast) {
        ...
        toast.className = 'vista-fichas toast toast-minimal';
    ...
    function initializeSelect2() {
        ...
        $('.vista-fichas .form-control-erp[multiple]').select2('destroy');
        $('.vista-fichas .form-control-erp:not([multiple])').select2('destroy');
        
        // Re-initialize Select2
        $('.vista-fichas .form-control-erp[multiple]').select2({
            theme: 'bootstrap4',
            width: '100%',
        ...
        $('.vista-fichas .form-control-erp:not([multiple])').not('[wire\\:ignore]').select2({
            theme: 'bootstrap4',
            width: '100%',
            placeholder: function() {
    ...
    document.addEventListener('keydown', function(event) {
        if (event.key === 'Escape') {
            const modals = document.querySelectorAll('.vista-fichas .modal-overlay');
    ...
    const searchInput = document.querySelector('.vista-fichas .search-input');
    if (searchInput) {
        searchInput.focus();
    }
    ...
    document.addEventListener('submit', function(e) {
        const form = e.target;
        if (form.classList.contains('ficha-form')) {
    ...
    document.addEventListener('click', function(e) {
        if (e.target.matches('.vista-fichas .modal-overlay')) {
            Livewire.dispatch('closeModal');
        }
    });
    ...
    document.addEventListener('input', function(e) {
        if (e.target.matches('.vista-fichas .search-input')) {
    ...
    document.addEventListener('change', function(e) {
        if (e.target.matches('.vista-fichas select[wire\\:model]')) {
            // Livewire will handle the update automatically through wire:model
        }
    });

    // Handle table row hover effects
    const tableRows = document.querySelectorAll('.vista-fichas .modern-table tbody tr');
    ...
    function handleResponsiveActions() {
        const table = document.querySelector('.vista-fichas .modern-table');
        if (table) {
    ...
    document.addEventListener('keydown', function(e) {
        if (e.target.closest('.vista-fichas .modern-table')) {
    ...
    window.addEventListener('beforeprint', function() {
        ...
        const elementsToHide = document.querySelectorAll('.vista-fichas .toolbar, .vista-fichas .btn-action');
        elementsToHide.forEach(el => el.style.display = 'none');
    });

    window.addEventListener('afterprint', function() {
        // Restore elements after printing
        const elementsToRestore = document.querySelectorAll('.vista-fichas .toolbar, .vista-fichas .btn-action');
    ...
    function enhanceAccessibility() {
        ...
        const modals = document.querySelectorAll('.vista-fichas .modal-container');
        modals.forEach(modal => {
        ...
        const actionButtons = document.querySelectorAll('.vista-fichas .btn-action');
    ...
    console.log('✅ Fichas - Todos los sistemas modernos listos');
});
