    ...
    @auth
        ...
        <div class="toolbar">
            ...
            <div class="filters-container">
                @haspermission('CAMBIAR ESTADO APRENDIZ')
                ...
                <select wire:model.live="fichaFilter" class="filter-select">
                    ...
                    @foreach ($fichas as $ficha)
                        <option value="{{ $ficha->id }}">{{ $ficha->ficha }} -
                            {{ $ficha->programaFormacion->nombre ?? 'Sin programa' }}</option>
                    @endforeach
                </select>
                ...
                <tbody>
                    @forelse ($aprendices as $aprendiz)
                        <tr>
                            <td class="codigo">
                                <span
                                    class="badge-modern badge-primary">{{ ($aprendices->currentPage() - 1) * $aprendices->perPage() + $loop->iteration }}</span>
                            </td>
                            ...
                            <td class="ficha">
                                @if ($aprendiz->fichaCaracterizacion)
                                    <span
                                        class="badge-modern badge-info">{{ $aprendiz->fichaCaracterizacion->ficha }}</span>
                                @else
                            ...
                            <td class="programa">
                                @if ($aprendiz->fichaCaracterizacion?->programaFormacion)
                                    <span
                                        class="badge-modern badge-success">{{ $aprendiz->fichaCaracterizacion->programaFormacion->nombre }}</span>
                                @else
                            ...
                            <td class="regional">
                                @if ($aprendiz->fichaCaracterizacion?->sede?->regional)
                                    <span
                                        class="badge-modern badge-primary">{{ $aprendiz->fichaCaracterizacion->sede->regional->nombre }}</span>
                                @else
                            ...
                            @haspermission('CAMBIAR ESTADO APRENDIZ')
                                <td class="estado">
                                    <button wire:click="toggleStatus({{ $aprendiz->id }})" wire:loading.attr="disabled"
                            ...
                            <td class="td-actions sticky-actions">
                                <button wire:click="openShowModal({{ $aprendiz->id }})" class="btn-action btn-view">
                                    <i class="fas fa-eye"></i>
                                </button>

                                @haspermission('EDITAR APRENDIZ')
                                    <button wire:click="openEditModal({{ $aprendiz->id }})" class="btn-action btn-edit">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                @endhaspermission

                                @haspermission('ELIMINAR APRENDIZ')
                                    <button wire:click="openDeleteModal({{ $aprendiz->id }})" class="btn-action btn-delete">
            ...
            <div class="modal-overlay" wire:click="closeCreateEditModals">
                <div class="modal-container modal-xl" wire:click.stop>
                    ...
                    <div class="modal-body">
                        <livewire:aprendices.aprendiz-form :aprendiz="$selectedAprendiz" :is-edit="$showEditModal" />
                    </div>
                </div>
            </div>
        @endif
                    ...
                    <div class="modal-body">
                                ...
                                <div>
                                    ...
                                    <div style="font-size: 14px; color: #1f2937; font-weight: 500;">
                                        @if ($selectedAprendiz->fichaCaracterizacion)
                                            <span
                                                class="badge-modern badge-info">{{ $selectedAprendiz->fichaCaracterizacion->ficha }}</span>
                                        @else
                                            <span class="badge-modern badge-secondary">Sin ficha</span>
                                        @endif
                                    </div>
                        ...
                        <div class="modal-section">
                            ...
                            <div style="display: flex; align-items: center; gap: 12px;">
                                <span
                                    class="badge-modern {{ $selectedAprendiz->estado ? 'badge-success' : 'badge-danger' }}">
                        ...
                        <div class="modal-section">
                            ...
                            <div style="display: grid; gap: 8px;">
                                ...
                                <div><strong>Documento:</strong>
                                    {{ $selectedAprendiz->persona?->numero_documento ?? 'N/A' }}</div>
                                <div><strong>Ficha:</strong>
                                    {{ $selectedAprendiz->fichaCaracterizacion?->ficha ?? 'Sin ficha' }}</div>
                            </div>
                        </div>
